...
def test_display_words(mock_stdscr):
    ...
    positions = [1, 2, 3]
    visible_count = handler.display_words(mock_stdscr, positions, DisplayState(0, 0, False))
...
def test_scroll_indicators(mock_stdscr):
    ...
    viewport = Viewport(height=10, width=80, start=1, end=2)
    handler._add_scroll_indicators(mock_stdscr, viewport, 3)
...
def test_display_words_with_masking(mock_stdscr):
    ...
    # Test masked display
    handler.display_words(mock_stdscr, positions, DisplayState(0, 0, False))
...
def test_display_boundaries(mock_stdscr):
    ...
    result = handler.display_words(mock_stdscr, [1, 2, 3], DisplayState(0, 0, False))
